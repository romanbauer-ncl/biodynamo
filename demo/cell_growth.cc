...
void execute(size_t cells_per_dim, size_t iterations, size_t threads,
 size_t repititions, TimingAggregator* statistic, bool with_export) {
  for (size_t r = 0; r < repititions; r++) {
    ...
    if (with_export) {
      ...
      Exporter exporter;
      exporter.conductExportToFile(&cells, "FinalPositions.dat");
      exporter.conductExportToMatlabFile(&cells, "FinalPositions.m");
      exporter.conductNeuroMLExportToFile(&cells, "FinalPositions.xml");
    }
...
//else if (args == 2 && (std::string(argv[1]) == "export" ||
//  std::string(argv[1]) == "--export")) {
//  std::cout << "information on cells..." << std::endl;
//  execute(8, 1, 1, 3, &statistic, true); 
//} 
else if (args >= 4) {
  size_t cells;
  size_t iterations;
